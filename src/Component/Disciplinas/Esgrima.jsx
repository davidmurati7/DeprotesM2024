...
import logoN from '../Home/esgrima2.jpg'; 
...
const data = [
  { id: 1,  Dia: "lunes", Hora: "4 pm", CompetidorA: "A", CompetidorB: "B", Disciplina: "", Ganador: "" },
];

const clave="12345"

class Esgrima extends React.Component {
  ...
  async getFutbolC() {
    try {
      const { data, error } = await supabase.from("Esgrima").select("*");
      if (error) throw error;
      this.setState({ data });
    } catch (error) {
      console.error("Error al obtener datos de Esgrima:", error.message);
    }
  }
  ...
  async updateFutbolC() {
    try {
      ...
      console.error("Error al actualizar datos de Esgrima:", error.message);
    }
  }
  ...
  async crearFutbolC() {
    try {
      ...
      const { data, error } = await supabase.from("Esgrima").insert([nuevoDato]);
      ...
      console.error("Error al crear datos en Esgrima:", error.message);
    }
  }
  ...
  async deleteFutbolC(dato) {
    try {
      ...
      console.error("Error al eliminar datos de Esgrima:", error.message);
    }
  }
  ...
  editar = async (dato) => {
    ...
    arreglo.map((registro) => {
      if (dato.id == registro.id) {
        ...
        arreglo[contador].Resultado = dato.Disciplina;
